...
```
docker pull deegree/deegree3-docker:3.6
```
...
```
docker run --name deegree_inspire_soil --network="inspiresoilnetwork" -d -p 8080:8080 deegree/deegree3-docker:3.6
```
...
To use the previously generated `Soil.sql`, a database needs to be created. For this example a docker container with a PostgreSQL image (version 18) and the PostGIS extension (version 3.6) is created:

```
docker run --name postgis_inspire_soil --network="inspiresoilnetwork" -d -p 5432:5432 -e POSTGRES_PASSWORD=postgres -e POSTGRES_USER=postgres -v /destination/Soil.sql:/ postgis/postgis:18-3.6
```
